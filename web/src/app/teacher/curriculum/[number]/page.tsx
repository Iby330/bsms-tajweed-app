...
export default async function HomeworkResults({
  ...
  searchParams: Promise<{ tab?: string; student?: string; class?: string }>;
}) {
  const [{ number }, { tab, student, class: classParam }] = await Promise.all([
    params,
    searchParams,
  ]);
  ...
  const path = `/teacher/curriculum/${hw.number}`;
  const tabHrefs = {
    summary: scopedHref(scope, path),
    question: scopedHref(scope, `${path}?tab=question`),
    individual: scopedHref(scope, `${path}?tab=individual`),
  };
  const individualHref = (studentId: string) =>
    scopedHref(scope, `${path}?tab=individual&student=${studentId}`);
